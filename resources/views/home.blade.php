...
@section('content')
    <main class="flex flex-col min-h-[calc(100vh-64px)] px-10">
        <section class="flex justify-center items-center relative h-64">
            <img class="absolute w-full h-full object-cover z-[-1] brightness-50" src="{{ asset('img/banner.jpg') }}" alt="banner">
        ...
        <section class="flex flex-col py-5 relative m-7">
            <x-boxes-background class="absolute z-[-1] w-full h-full"></x-boxes-background>
            <h2 class="text-2xl dark:text-primary-dark text-primary-light px-5">Articulos</h2>
            <p class="px-5 dark:text-white text-black">Articulos mas recientes</p>
            <div class="flex gap-3 flex-wrap w-3/4 mt-5">
                @if (isset($posts) && count($posts) > 0)
                    @foreach ($posts as $post)
                        <article class="flex gap-3 backdrop-blur dark:bg-[rgba(255,255,255,0.04)] bg-[rgba(0,0,0,0.2)] rounded-md w-full p-3 overflow-x-hidden">
